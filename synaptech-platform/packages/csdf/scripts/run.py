...
def run_single_node(steps=50, use_cosmos=False):
    ...
    if use_cosmos:
        from consciousness.cosmos_bridge import CosmosEmbeddingEnv
        env = CosmosEmbeddingEnv()
        agent = Agent(node_id=0, env=env)
        print("  Using Cosmos-backed embedding environment")
    else:
...
def main():
    ...
    parser.add_argument("--cosmos", action="store_true", help="Use Cosmos-backed embedding environment")
    args = parser.parse_args()

    if args.train_lang:
        run_language_training(steps=args.train_steps, corpus=args.corpus)
    elif args.single:
        run_single_node(steps=args.steps, use_cosmos=args.cosmos)
    else:
