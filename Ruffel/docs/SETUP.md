...
| | |
|--|--|
| **OS** | Ubuntu 26.04 LTS (recommended) · 25.10 · **Arch Linux** (fully supported) |
| **GPU mode** | NVIDIA GPU · 12 GB VRAM minimum · 24 GB recommended |
| **CPU mode** | 24 GB RAM |
| **Ollama mode** | Any GPU or CPU (uses existing Ollama instance) |
| **Disk** | ~20 GB free (llama.cpp) · **0 GB** (Ollama provider mode) |
...
On Arch Linux, the installer defaults to **Ollama provider mode** — no model download needed:

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ShrekDino/OpenMonoAgent.ai/refs/heads/main/get-openmono.sh)
```

The prerequisite installer uses **pacman** and installs: Docker, .NET 10, git, jq, ripgrep, cmake, base-devel. NVIDIA drivers are optional.

Prerequisites are installed automatically via `scripts/install_prereqs_arch.sh`. The installer builds only the agent image (no llama-server), configures Ollama as the provider, and sets up shell integration.

For full llama.cpp inference mode on Arch:
```bash
openmono setup --full
```

→ [Full Arch Linux guide](../ARCHLINUX.md)

---

## Step 2 — During setup (Ubuntu)
