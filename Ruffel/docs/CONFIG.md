...
| Command | Description |
|---------|-------------|
| `openmono agent` | Run the coding agent in the current directory (terminal REPL mode) |
| `openmono --rpc` | Run the coding agent in JSON-RPC mode (for terminal client frontend) |
| `openmono --vscode` | Run the coding agent in VS Code extension mode |
| `openmono --tui` | Run the coding agent in full-screen TUI mode |
| `openmono --classic` | Run the coding agent in classic scrolling terminal mode |
...
```jsonc
"providers": {
  "ollama":    { "endpoint": "http://localhost:11434", "model": "qwen3.5:9b",  "active": true },
...
### Ollama Provider (Recommended for lightweight setups)

The Ollama provider lets you use your existing [Ollama](https://ollama.ai) instance as the LLM backend — no model download, no llama.cpp server, GPU-agnostic.

**Advantages:**
- Works on any hardware Ollama supports (including 4-6GB VRAM GPUs)
- No 15-18GB model download
- Already-running Ollama instance is shared with OpenCode
- Hot-swappable models via `/model` in TUI
...
The endpoint defaults to `http://localhost:11434` — Ollama's default port. In Docker, use `http://host.docker.internal:11434` or enable `network_mode: host` on the agent service.

**Model management:**
```bash
ollama list                          # List available models
openmono config set llm.model qwen3.5:9b  # Switch model
/model qwen3.5:9b                    # Switch mid-session (TUI)
```
