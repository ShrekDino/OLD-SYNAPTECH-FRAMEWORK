...
for (var i = 0; i < args.Length; i++)
{
    ...
    switch (arg)
    {
        ...
        case "--vscode": useTui = false; break;
        case "--rpc": useTui = false; break;
        case "--help" or "-h":
...
static async Task RunAgentAsync(string? endpoint, string? model, string? workdir, string? configPath, bool verbose = false, bool showDetail = false, bool? useTui = null)
{
    ...
    VscodeJsonRpcServer? rpcServer = null;
    VscodeRenderer? vsRenderer = null;
    TerminalJsonRpcServer? terminalRpcServer = null;
    OpenCodeBridgeRenderer? ocbRenderer = null;
    ...
    if (enableTui && config.Renderer != RendererMode.Vscode && config.Renderer != RendererMode.Rpc)
    {
    ...
    if (config.Renderer == RendererMode.Vscode)
    {
        vsRenderer = new VscodeRenderer();
        vsRenderer.Verbose = config.Verbose;
        var toolDispatcher = new ToolDispatcher(tools, permissions, vsRenderer, config, session);
        rpcServer = new VscodeJsonRpcServer(config, sessionManager, session, toolDispatcher, vsRenderer);
        vsRenderer.SetServer(rpcServer);
        renderer = vsRenderer;
    }

    if (config.Renderer == RendererMode.Rpc)
    {
        ocbRenderer = new OpenCodeBridgeRenderer();
        ocbRenderer.Verbose = config.Verbose;
        var toolDispatcher = new ToolDispatcher(tools, permissions, ocbRenderer, config, session);
        terminalRpcServer = new TerminalJsonRpcServer(config, sessionManager, session, toolDispatcher, ocbRenderer);
        ocbRenderer.SetServer(terminalRpcServer);
        renderer = ocbRenderer;
    }

    var loop = new ConversationLoop(llm, tools, permissions, renderer, renderer, renderer, config, session, compactor, memoryStore,
        checkpointer: checkpointer);

    if (rpcServer is not null)
        rpcServer.SetLoop(loop);
    if (terminalRpcServer is not null)
        terminalRpcServer.SetLoop(loop);
    ...
    if (config.Renderer == RendererMode.Rpc)
    {
        ...
        await terminalRpcServer!.ServeAsync(CancellationToken.None);
