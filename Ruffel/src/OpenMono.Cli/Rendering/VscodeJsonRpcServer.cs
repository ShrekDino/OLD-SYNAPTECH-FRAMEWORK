...
public sealed class VscodeJsonRpcServer : IDisposable
{
    ...
    private readonly VscodeRenderer _renderer;
    ...
    public VscodeJsonRpcServer(
        AppConfig config,
        SessionManager sessionManager,
        SessionState session,
        ToolDispatcher toolDispatcher,
        VscodeRenderer renderer)
    {
        _stdin = Console.In;
        _stdout = Console.Out;
        _config = config;
    ...
    private async Task<JsonElement> HandleInputSend(JsonElement @params, CancellationToken ct)
    {
        ...
        await _loop.RunTurnAsync(text, ct);
