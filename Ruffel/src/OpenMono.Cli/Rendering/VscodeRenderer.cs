...
public sealed class VscodeRenderer : IRenderer
{
    private VscodeJsonRpcServer _server;
    private readonly ConcurrentDictionary<string, TaskCompletionSource<PermissionResponse>> _pendingPermissions = new();

    public bool Verbose { get; set; }

    public VscodeRenderer()
    {
        _server = null!;
    }

    public VscodeRenderer(VscodeJsonRpcServer server)
    {
        _server = server;
    }

    public void SetServer(VscodeJsonRpcServer server) => _server = server;

    public void EnableCommandSuggestions(CommandRegistry registry) { }

    public string ReadInput()
    {
        throw new NotSupportedException("VscodeRenderer does not support ReadInput — use JSON-RPC input/send instead.");
    }
