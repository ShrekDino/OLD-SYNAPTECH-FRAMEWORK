...
public static class ConfigLoader
{
    ...
    private static void ApplyEnvironmentOverrides(AppConfig config)
    {
        ...
        var renderer = Environment.GetEnvironmentVariable("OPENMONO_RENDERER");
        if (!string.IsNullOrEmpty(renderer) && Enum.TryParse<RendererMode>(renderer, true, out var rendererMode))
            config.Renderer = rendererMode;
    }
}
